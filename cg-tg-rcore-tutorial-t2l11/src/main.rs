...
const PRIMARY_WAITING: usize = 0x5A5A_8001;
const PRIMARY_READY: usize = 0x5A5A_8002;
...
fn primary_rust_main() -> ! {
    ...
    log::info!("all secondary harts are parked; boot hart continues with thread scheduling");
    // 步骤 3：堆分配器
...
fn announce_secondary_hart(hart_id: usize) {
    ...
    writeln!(
        &mut writer,
        "hart {hart_id} is online but parked before thread scheduling"
    )
    .unwrap();
}
...
mod impls {
    ...
    impl tg_console::Console for Console {
        ...
        fn put_char(&self, c: u8) {
            let _guard = crate::CONSOLE_LOCK.lock();
