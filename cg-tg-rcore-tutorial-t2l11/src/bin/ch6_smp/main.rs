...
const PRIMARY_WAITING: usize = 0x5A5A_6001;
const PRIMARY_READY: usize = 0x5A5A_6002;
...
fn primary_rust_main() -> ! {
    ...
    log::info!("all secondary harts are parked; boot hart continues with filesystem init");
    // 步骤 3：初始化内核堆分配器
...
fn announce_secondary_hart(hart_id: usize) {
    ...
    writeln!(
        &mut writer,
        "hart {hart_id} is online but parked before filesystem scheduling"
    )
    .unwrap();
}
