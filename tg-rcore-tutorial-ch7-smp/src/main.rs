...
const PRIMARY_WAITING: usize = 0x5A5A_7001;
const PRIMARY_READY: usize = 0x5A5A_7002;
...
fn primary_rust_main() -> ! {
    ...
    log::info!("all secondary harts are parked; boot hart continues with pipe and signal scheduling");
    // 步骤 3：初始化内核堆分配器
...
fn announce_secondary_hart(hart_id: usize) {
    ...
    writeln!(
        &mut writer,
        "hart {hart_id} is online but parked before pipe and signal scheduling"
    )
    .unwrap();
}
