...
fn current_hartid() -> usize {
    let hart_id;
    // SAFETY: 读取 mhartid CSR 是合法的 M-mode 只读操作，用于选择当前 hart
    // 对应的定时器比较寄存器和调试行为。
    unsafe {
...
fn handle_timer(time: u64) -> SbiRet {
    ...
    let mtimecmp = CLINT_MTIMECMP_BASE + hart_id * core::mem::size_of::<u64>();

    // SAFETY: QEMU virt 为每个 hart 提供独立的 CLINT mtimecmp 槽位。
    // 使用当前 hart id 计算地址，避免多个 hart 抢写同一个比较寄存器。
    unsafe {
