...
fn build_apps_and_pack_fs() {
    let selected = env::var("CG_TG_CASE_SET").unwrap_or_else(|_| "ch8".to_string());
    if copy_bundled_fs_image(&selected) {
        return;
    }
...
fn copy_bundled_fs_image(selected: &str) -> bool {
    use std::io::Read;
    use xz2::read::XzDecoder;

    let manifest_dir = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").unwrap());
    let source = manifest_dir.join("assets").join(match selected {
        "ch6" => "fs_ch6.img.xz",
        _ => "fs_ch8.img.xz",
    });
    ...
    fs::create_dir_all(&fs_target_dir).unwrap_or_else(|err| {
        panic!(
            "failed to create bundled fs target dir {}: {err}",
            fs_target_dir.display()
        )
    });
    let target = fs_target_dir.join("fs.img");
    let file = fs::File::open(&source).unwrap_or_else(|err| {
        panic!(
            "failed to open bundled fs image {}: {err}",
            source.display()
        )
    });
    let mut decoder = XzDecoder::new(file);
    let mut image = Vec::new();
    decoder.read_to_end(&mut image).unwrap_or_else(|err| {
        panic!(
            "failed to decode bundled fs image {}: {err}",
            source.display()
        )
    });
    fs::write(&target, image).unwrap_or_else(|err| {
        panic!(
            "failed to write bundled fs image to {}: {err}",
            target.display()
        )
    });
    true
