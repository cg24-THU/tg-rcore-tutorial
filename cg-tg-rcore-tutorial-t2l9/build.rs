...
fn build_apps() {
    if let Some((base, step, bins)) = bundled_ch2_bins() {
...
fn bundled_ch2_bins() -> Option<(u64, u64, Vec<PathBuf>)> {
    let manifest_dir = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR")?);
    let assets_dir = manifest_dir.join("assets").join("ch2");
    println!("cargo:rerun-if-changed={}", assets_dir.display());

    let mut bins = Vec::new();
    for index in 0..=6 {
        let path = assets_dir.join(format!("ch2_draw_piece_{index}.bin"));
        if !path.exists() {
            return None;
        }
        bins.push(path);
    }
    Some((0x8200_0000, 0, bins))
}
...
fn objcopy_to_bin(elf: &PathBuf) -> PathBuf {
    ...
    let bytes = fs::read(elf).unwrap_or_else(|err| panic!("failed to read {}: {err}", elf.display()));
    let elf_file =
        ElfFile::new(&bytes).unwrap_or_else(|err| panic!("failed to parse {}: {err}", elf.display()));
    ...
    for header in elf_file.program_iter() {
        ...
        let end = cursor + mem_size;
        if image.len() < end {
            image.resize(end, 0);
        }
        image[cursor..cursor + file_size].copy_from_slice(&bytes[offset..offset + file_size]);
        cursor = end;
    }

    let mut out =
        fs::File::create(&bin).unwrap_or_else(|err| panic!("failed to create {}: {err}", bin.display()));
    out.write_all(&image)
        .unwrap_or_else(|err| panic!("failed to write {}: {err}", bin.display()));
    bin
}
