...
static int null_patch_warnings = 0;

static int V_CheckPatch(const char *func, patch_t *patch, int x, int y)
{
    if (patch != NULL)
    {
        return 1;
    ...
    if (null_patch_warnings < 8)
    {
        fprintf(stderr, "doom: %s null patch x=%d y=%d\n", func, x, y);
        ++null_patch_warnings;
    }
...
{ 
    ...
    if (!V_CheckPatch("V_DrawPatch", patch, x, y))
    {
        return;
    }
...
void V_DrawPatchFlipped(int x, int y, patch_t *patch)
{
    ...
    if (!V_CheckPatch("V_DrawPatchFlipped", patch, x, y))
    {
        return;
    }
...
void V_DrawTLPatch(int x, int y, patch_t * patch)
{
    ...
    if (!V_CheckPatch("V_DrawTLPatch", patch, x, y))
    {
        return;
    }
